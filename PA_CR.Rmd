...
This code generates the results and the main figures presented in the manuscript “Offspring number, size, and survival: state-dependent optimization of litter size in a long-lived capital breeder” *Ecological monographs*
...
get_Rhat <- function(nimble_output, params) {
  ...
  for (k in 1:length(params)) {
    posterior <- cbind(nimble_output$samples$chain1[, params[k]],
                       nimble_output$samples$chain2[, params[k]])
  ...
  for (i in 1:n_ind) {
    for (t in f[i]:(n_year-1)) {
      ...
      logit(phi[i, t]) <- 
        ...
        theta[17] * (1 - Age0[i, t]) * Sex[i] +                                # Effect of sex = male for individuals older than cubs
        theta[18] * ((1 - Age0[i, t]) * (1 - Sex[i]) * mSpUnk[i] +          # Effect of space use = NA for females >0yr 
                       Age1[i, t] * Sex[i] * mSpUnk[mother[i]]) +             # Effect of maternal space use = NA for male yearlings 
        theta[19] * ((1 - Age0[i, t]) * (1 - Sex[i]) * mSpPel[i] +     # Effect of space use = pelagic for females >0yr
...
ggplot(data = data.frame(x = plogis(rnorm(15000, mean = 0, sd = 1))),
       aes(x = x)) +
  geom_histogram(fill = "grey75", alpha = 0.75) +
  theme_bw() +
  labs(x = "cub survival", subtitle = "N(0, 1)") +
ggplot(data = data.frame(x = plogis(rnorm(15000, mean = 0, sd = 1.5))),
       aes(x = x)) +
  geom_histogram(fill = "grey75", alpha = 0.75) +
  theme_bw() +
  labs(x = "cub survival", subtitle = "N(0, 1.5)")
```
...
n_ind <- length(unique(CR_data$ID_NR))

years_obs <- unique(CR_data$year)
year <- as.character(min(years_obs):max(years_obs))
n_year <- length(year)

# Generate the capture histories
capt_hist_1 <- reshape2::dcast(CR_data, ID_NR ~ year, fun.aggregate = length, fill = 0)

# The column for 2020 is missing since there were no captures this year. Let's add it
capt_hist_2 <- cbind(capt_hist_1[, 1:29], rep(0, times = n_ind), capt_hist_1[, 30:32])
colnames(capt_hist_2)[30] <- "2020"

# Get the age at first capture of each
age_first <- CR_data %>%
  group_by(ID_NR) %>%
  filter(rank(age_for_analyses, ties.method = "first") == 1) %>%
  dplyr::select(ID_NR, age_first = age_for_analyses)

# Sort by age at first capture, to have cubs first
capt_hist_2_df <- as.data.frame(capt_hist_2) %>%
  left_join(x = .,
            y = age_first,
            by = "ID_NR") %>%
  arrange(age_first, ID_NR)

capt_hist_3 <- as.matrix(capt_hist_2_df %>% dplyr::select(-age_first))
rm(capt_hist_1, capt_hist_2)

# Remove column with IDs
capt_hist <- capt_hist_3[, -1]
```
...
f <- NULL
for (i in 1:n_ind){
  f[i] <-  which(capt_hist[i, ] == 1)[1]
}

# Create matrices for age group
AGE <- mAgeY <- mAgeM <- mAgeO <- Age0 <- Age1 <- Age2 <- Age3_4 <- Age5_19 <- Age20 <- matrix(NA, ncol = dim(capt_hist)[2], nrow = dim(capt_hist)[1])
colnames(Age0) <- colnames(Age1) <- colnames(Age2) <- colnames(Age3_4) <- colnames(Age5_19) <- colnames(Age20) <- year

for (i in 1:dim(capt_hist)[1]){
  ID_i = capt_hist_3[i, 1]
  year_first = colnames(capt_hist)[f[i]]
  age <- CR_data$age_for_analyses[ which(CR_data$ID_NR == ID_i & CR_data$year == year_first) ]
  for (t in f[i]:dim(capt_hist)[2]) {
...
has_a_litter <- matrix(data = 0, nrow = dim(capt_hist)[1], ncol = dim(capt_hist)[2],
                       dimnames = list(1:dim(capt_hist)[1], 1992:2023))
Length <- Girth <- matrix(data = NA, nrow = dim(capt_hist)[1], ncol = dim(capt_hist)[2],
                        dimnames = list(1:dim(capt_hist)[1], 1992:2023))
LS <- LS1 <- LS2 <- LS3 <- matrix(data = 0, nrow = dim(capt_hist)[1], ncol = dim(capt_hist)[2],
                                  dimnames = list(1:dim(capt_hist)[1], 1992:2023))

for (i in 1:nrow(CR_data)) {
  if (CR_data$cub_status_complete[i] %in% c("c")) {
    has_a_litter[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- 1
    
    Length[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- CR_data$s_length[i]
    Girth[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- CR_data$girth[i]
    LS[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- CR_data$cub_number[i]
    LS1[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- ifelse(CR_data$cub_number[i] == 1, 1, 0)
    LS2[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- ifelse(CR_data$cub_number[i] == 2, 1, 0)
    LS3[which(capt_hist_3[, 1] == CR_data$ID_NR[i]), as.character(CR_data$year[i])] <- ifelse(CR_data$cub_number[i] == 3, 1, 0)
  }
}
...
row_litter <- NULL
col_litter <- NULL
for (i in 1:dim(has_a_litter)[1]) {
  for (t in 1:dim(has_a_litter)[2]) {
...
# Create matrices with cub mass, litter size and line of mother
mass_cub <- LS_focal <- matrix(data = 0, nrow = dim(capt_hist)[1], ncol = dim(capt_hist)[2])
ID_mother <- rep(NA, times = dim(capt_hist)[1])
mother <- rep(411, times = dim(capt_hist)[1]) # I use 411 because there is no NA on that line
# in age. This way, there won't be NAs in the model
for (i in 1:dim(capt_hist)[1]) {
  # Get age at first capture
  if (Age0[i, f[i]] == 1) {
    ID_i = capt_hist_3[i, 1]
    mass_cub[i, f[i]] <- CR_data$mass[ which(CR_data$ID_NR == ID_i & CR_data$age_for_analyses == 0) ]
    LS_focal[i, f[i]] <- CR_data$litter_size[ which(CR_data$ID_NR == ID_i & CR_data$age_for_analyses == 0) ] # There are 2 NAs in the LS_focal matrix. They correspond to the two cubs (23263 & 23264) that were captured with the adopted third cub (23231)
    #  Mother
    ID_mother[i] <- CR_data$ID_mat[ which(CR_data$ID_NR == ID_i & CR_data$age_for_analyses == 0) ]
    mother[i] <- which(capt_hist_3[, 1] == ID_mother[i])
  }
}
mass_cub_vector <- apply(mass_cub, 1, max)             # vector with the mass of each cub (except cubs that weren't weighed)
mass_cub_vector[mass_cub_vector == 0] <- NA
log_mass_cub_vector <- log(mass_cub_vector)
M <- as.vector(scale(log_mass_cub_vector))
M[is.na(M)] <- 0

save(log_mass_cub_vector, 
     file = "data/log_mass.RData")

LS_cub_focal_vector <- apply(LS_focal, 1, max)


# Create:
# - a matrix with the day of capture
# - vectors withsex and space-use strategy
DayCapture <- matrix(data = NA, nrow = dim(capt_hist)[1], ncol = dim(capt_hist)[2])
Sex <- mSpUnk <- mSpPel <- NULL
for (i in 1:dim(capt_hist)[1]) {
  ID_i = capt_hist_3[i, 1]
  Sex[i] <- ifelse(CR_data$sex[which(CR_data$ID_NR == ID_i)][1] %in% "m", 1, 0) # 1 cub with unknown sex, assigned to females.
  mSpUnk[i] <- ifelse(is.na(CR_data$ecot[which(CR_data$ID_NR == ID_i)][1]), 1, 0)
  mSpPel[i] <- ifelse(CR_data$ecot[which(CR_data$ID_NR == ID_i)][1] %in% "O", 1, 0)
  
  for (t in f[i]:dim(capt_hist)[2]) {
    if (capt_hist[i, t] %in% 1) {
      DayCapture[i, t] <- CR_data$day_number[ which(CR_data$ID_NR == ID_i & CR_data$year == (t+1991)) ]
    }
  }
}
...
# Let's get the coordinates of the cubs
col_cub <- NULL
for (i in 1:dim(Age0)[1]) {
  for (t in 1:dim(Age0)[2]) {
    if (Age0[i, t] %in% 1) {
      col_cub <- c(col_cub, t)
    }
  }
...
# Function to create known states
# We want ones where we known an individual is alive (between its first and last capture)
# and NAs elsewhere. We also need to put an NA in the first capture.
...
dat <- list(mL = mL,
            ...
            M = M,
            y = capt_hist,
            z = known.state.cjs(capt_hist))

my.constants <- list(n_year = n_year,
                     n_ind = n_ind,
                     # Length +++++++++++++++++++++++++++++++++++++
                     n_coef_mL = n_coef_mL,
...
# We want NAs everywhere until the last capture
ch.init <- function(ch, f) {
  for (i in 1:dim(ch)[1]) {
    last <- max(which(ch[i, ] == 1))
    ch[i, 1:last] <- NA # ch[i, 1:f[i]] <- NA
  }
  return(as.matrix(ch))
}
...
# set.seed(2023)
# fit_PA_CR <-  nimbleMCMC(code = PA_CR,
#                                        data = dat,
...
# save_plot(filename = "outputs//fit_PA_CR.png",
...
model_df <- data.frame(name = c("alpha[1]", "alpha[2]", "alpha[3]", "alpha[4]", "alpha[5]", "sigma_zeta",
                       ...
                       covariate = c("intercept", "intercept", "intercept", "ecot NA", "ecot pelagic",
                                     ...
                                     size2 twin", "DayCapture twin", "ecot NA twin", "ecot pelagic twin",
                                     ...
                                     "survival 1yr", "survival 2yr", "survival 3-4yr", "survival 5-19yr",
                       ...
                       age = c("young", "middle-aged", "old", NA, NA, NA, NA,
                               ...
                               # CR
                               "young", "middle-aged", "old", rep(NA, times = 21)),
                       submodel = c(rep("Maternal size", 7), rep("Litter size", 13), rep("Maternal girth", 11), rep("Cub mass", 23), rep("Cub survival", times = 24)))

load("outputs/fit_PA_CR.RData")

N <- dim(fit_PA_CR$samples$chain1)[1]
res <- rbind(fit_PA_CR$samples$chain1[seq(1, N, by = 10), ],
             fit_PA_CR$samples$chain2[seq(1, N, by = 10), ])
cols <- model_df$name
...
ggplot(data = caterpillar,
  ...
  labs(x = "", y = "", color = "submodel",
       subtitle = paste0("wAIC = ", round(fit_PA_CR$WAIC$WAIC, 2))) +
  coord_flip()
```
...
```{r}
res <- rbind(fit_PA_CR$samples$chain1,
             fit_PA_CR$samples$chain2)

girth <- array(NA, dim = c(dim(res)[1], 3, 3),
               dimnames = list(1:dim(res)[1], c("with singleton", "with twin", "with triplet"), c("young", "middle-aged", "old")))
cub_mass <- array(NA, dim = c(dim(res)[1], 3, 3),
                  dimnames = list(1:dim(res)[1], c("singleton", "twin", "triplet"), c("young", "middle-aged", "old")))
surviving_cubs <- array(NA, dim = c(dim(res)[1], 3, 3),
                        dimnames = list(1:dim(res)[1], c("singleton", "twin", "triplet"), c("young", "middle-aged", "old")))

intercept_size <- c("alpha[1]", "alpha[2]", "alpha[3]")
twin <- c(0, 1, 0)
triplet <- c(0, 0, 1)
intercept_girth <- c("gamma[1]", "gamma[2]", "gamma[3]")
intercept_mass_singleton <- c("delta[1]", "delta[2]", "delta[3]")
intercept_mass_twin <- c("delta[10]", "delta[11]", "delta[12]")
intercept_mass_triplet <- c("delta[13]", "delta[14]", "delta[15]")
intercept_survival <- c("theta[1]", "theta[2]", "theta[3]")
...
for (i in 1:3) {                # maternal age
  for (j in 1:3) {              # litter size
    for (k in 1:dim(res)[1]) {
      ...
      girth[k, j, i] <- res[k, intercept_girth[i]] +                  # intercept
        res[k, "gamma[4]"] * res[k, intercept_size[i]] +         # effect of size
        res[k, "gamma[6]"] * twin[j] +                           # effect twin litter
        res[k, "gamma[7]"] * triplet[j]                          # effect triplet litter
      # AFTER RE-RUNNING THE MODEL, NEED TO CHANGE gamma[6] to gamma[7]
      # Singleton ++++++++++++++++++++++++++++
      if (j == 1) {
        cub_mass[k, j, i] <- (res[k, intercept_mass_singleton[i]] + # If female cub
                                res[k, "delta[4]"] * res[k, intercept_size[i]] +
                                res[k, "delta[5]"] * girth[k, j, i]) * 0.5 +

          (res[k, intercept_mass_singleton[i]] +                     # If male cub
             res[k, "delta[4]"] * res[k, intercept_size[i]] +
             res[k, "delta[5]"] * girth[k, j, i] +
             res[k, "delta[7]"]) * 0.5

        surviving_cubs[k, j, i] <- ilogit(res[k, intercept_survival[i]] +
                                            res[k, "theta[4]"] * cub_mass[k, j, i] +           # Singleton mass
                                            res[k, "theta[5]"] * (twin[j] + triplet[j]) +    # Singleton litter
                                            res[k, "theta[6]"] * res[k, intercept_size[i]] + # Effect of size
                                            res[k, "theta[7]"] * girth[k, j, i])             # Effect of girth (with one cub)
      } else {

        # Twins ++++++++++++++++++++++++++++++++++
        if (j == 2) {
          cub_mass[k, j, i] <- (res[k, intercept_mass_twin[i]] +       # If female cub
                                  res[k, "delta[16]"] * res[k, intercept_size[i]] +
                                  res[k, "delta[17]"] * girth[k, j, i]) * 0.5 +

            (res[k, intercept_mass_twin[i]] +                          # If male cub
               res[k, "delta[16]"] * res[k, intercept_size[i]] +
               res[k, "delta[17]"] * girth[k, j, i] +
               res[k, "delta[19]"]) * 0.5

          surviving_cubs[k, j, i] <- 2 * ilogit(res[k, intercept_survival[i]] +
                                                  res[k, "theta[4]"] * cub_mass[k, j, i] +        # Twin mass
                                                  res[k, "theta[5]"] * (twin[j] + triplet[j]) +   # twin litter
                                                  res[k, "theta[6]"] * res[k, intercept_size[i]] +
                                                  res[k, "theta[7]"] * girth[k, j, i])            # Girth with large litter
        } else {

          # Triplets ++++++++++++++++++++++++++++++++++
          if (j == 3) {
            cub_mass[k, j, i] <- (res[k, intercept_mass_triplet[i]] +   # If female cub
                                    res[k, "delta[16]"] * res[k, intercept_size[i]] +
                                    res[k, "delta[17]"] * girth[k, j, i]) * 0.5 +

              (res[k, intercept_mass_triplet[i]] +                      # If male cub
                 res[k, "delta[16]"] * res[k, intercept_size[i]] +
                 res[k, "delta[17]"] * girth[k, j, i] +
                 res[k, "delta[19]"]) * 0.5

            surviving_cubs[k, j, i] <- 3 * ilogit(res[k, intercept_survival[i]] +
                                                    res[k, "theta[4]"] * cub_mass[k, j, i] +       # Twin mass
                                                    res[k, "theta[5]"] * (twin[j] + triplet[j]) +  # triplet litter
                                                    res[k, "theta[6]"] * res[k, intercept_size[i]] +
                                                    res[k, "theta[7]"] * girth[k, j, i])           # Girth with large litter
...
load("outputs/fit_PA_CR.RData")
N <- dim(fit_PA_CR$samples$chain1)[1]
res <- rbind(fit_PA_CR$samples$chain1[seq(1, N, by = 5), ],
             fit_PA_CR$samples$chain2[seq(1, N, by = 5), ])

M <- M[1:410]
log_mass_cub_vector <- log_mass_cub_vector[1:410]

residuals_age_litter_size <- matrix(data = NA, nrow = dim(res)[1], ncol = length(M),
                                    dimnames = list(1:dim(res)[1], 1:length(M)))
age_mat <- NULL
...
residuals_age_litter_size_df <- as.data.frame.table(exp(unscale(residuals_age_litter_size, log_mass_cub_vector))) %>%
  ...
  mutate(cub_number = LS_cub_focal_vector[1:410],
...
residuals <- matrix(data = NA, nrow = dim(res)[1], ncol = length(M),
                                    dimnames = list(1:dim(res)[1], 1:length(M)))
...
yearly_rd_eff <- array(data = NA, dim = c(dim(res)[1], 32, 4),
                       dimnames = list(1:dim(res)[1], 1:32, c("litter size", "maternal girth", "cub mass", "recapture probability")))
...
```{r}
# Maternal length +++++++++++++++
length <- length_with_rd_eff <- matrix(NA, nrow = dim(res)[1], ncol = n_year,
                dimnames = list(1:dim(res)[1], 1:n_year))
for (t in 1:n_year) {         # Year
  for (j in 1:dim(res)[1]) {       # MCMC iteration
    length_with_rd_eff[j, t] <- res[j, "alpha[1]"] +
      res[j, paste0("zeta[", t, "]")]
    
    length[j, t] <- res[j, "alpha[1]"] 
  }
}

cohort_rd_eff <- unscale(length_with_rd_eff, CR_data$s_length) - unscale(length, CR_data$s_length)

df_plot_cohort <- as.data.frame.table(cohort_rd_eff) %>%
  rename(iteration = Var1, cohort = Var2, value = Freq) %>%
...
```{r}
# Litter size ++++++++++++++++
log_twins <- log_twins_with_rd_eff <- log_triplets <- matrix(data = NA, nrow = dim(res)[1], ncol = n_year,
                                                          dimnames = list(1:dim(res)[1], 1:n_year))
for (t in 1:n_year) {    # year
  for (j in 1:dim(res)[1]) {
    log_twins_with_rd_eff[j, t] <- res[j, "beta[1]"] +
      res[j, paste0("eta[", t, "]")]
    log_triplets[j, t] <- res[j, "beta[9]"] 

    log_twins[j, t] <- res[j, "beta[1]"] 
    log_triplets[j, t] <- res[j, "beta[9]"] 

  }
}
prob_twins <- exp(log_twins) / (exp(0) + exp(log_twins) + exp(log_triplets))
prob_twins_with_rd_eff <- exp(log_twins_with_rd_eff) / (exp(0) + exp(log_twins_with_rd_eff) + exp(log_triplets))

yearly_rd_eff[, , "litter size"] <- prob_twins_with_rd_eff - prob_twins

# Maternal girth +++++++++++++++
girth <- girth_with_rd_eff <- matrix(NA, nrow = dim(res)[1], ncol = n_year,
                dimnames = list(1:dim(res)[1], 1:n_year))
for (t in 1:n_year) {         # Year
  for (j in 1:dim(res)[1]) {       # MCMC iteration
    girth_with_rd_eff[j, t] <- res[j, "gamma[1]"] +
      res[j, paste0("rho[", t, "]")]
    
    girth[j, t] <- res[j, "gamma[1]"] 
  }
}

yearly_rd_eff[, , "maternal girth"] <- unscale(girth_with_rd_eff, CR_data$girth) - unscale(girth, CR_data$girth)

# Cub mass +++++++++++++++
cub_mass <- cub_mass_with_rd_eff <- matrix(NA, nrow = dim(res)[1], ncol = n_year,
                                        dimnames = list(1:dim(res)[1], 1:32))
for (t in 1:n_year) {           # Year
  for (j in 1:dim(res)[1]) {        # MCMC iteration
    cub_mass_with_rd_eff[j, t] <- res[j, "delta[10]"] +
      
      res[j, paste0("nu[", t, "]")]
    
    cub_mass[j, t] <- res[j, "delta[10]"]
  }
}
yearly_rd_eff[, , "cub mass"] <-  exp(unscale(cub_mass_with_rd_eff, log_mass_cub_vector[1:410])) - exp(unscale(cub_mass, log_mass_cub_vector[1:410]))


# Recapture +++++++++++++++
recapture <- recapture_with_rd_eff <- matrix(NA, nrow = dim(res)[1], ncol = (n_year-1),
                                          dimnames = list(1:dim(res)[1], 1:(n_year-1)))
for (t in 1:(n_year-1)) {           # Year
  for (j in 1:dim(res)[1]) {        # MCMC iteration
    recapture_with_rd_eff[j, t] <- res[j, "lambda[1]"] +
      res[j, paste0("omega[", t, "]")]
    
    recapture[j, t] <- res[j, "lambda[1]"]
  }
}
yearly_rd_eff[, 2:32, "recapture probability"] <-  plogis(recapture_with_rd_eff) - plogis(recapture)


df_plot_year <- as.data.frame.table(yearly_rd_eff) %>%
  rename(iteration = Var1, year = Var2, trait = Var3, value = Freq) %>%
  mutate(year = droplevels(year),
         year = 1991 + as.numeric(year))
```

```{r, fig.height = 9, fig.width = 7}
cohort <- ggplot(df_plot_cohort, 
  ...
  labs(x = "year", y = "cohort random effect")
  
year <- ggplot(df_plot_year, 
       aes(x = as.factor(year), y = value)) +
  stat_summary(fun.data = get_mean_and_CI,
               fun.args = list(lower = 0.025, upper = 0.975),
               geom = "pointrange", 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(.~trait, ncol = 1, scales = "free_y") +
  labs(x = "year", y = "yearly random effect")
...
log_twins <- matrix(data = NA, nrow = dim(res)[1], ncol = 3,
                    dimnames = list(1:dim(res)[1], c("young", "middle-aged", "old")))
log_triplets <- matrix(data = NA, nrow = dim(res)[1], ncol = 3,
                       dimnames = list(1:dim(res)[1], c("young", "middle-aged", "old")))
...
# Effect of ecotype NA
get_probability_direction(res[, "beta[7]"])

# Effect of ecotype pelagic
...
prop_age_class_sex <- rev(data_cub %>% 
                            mutate(age_category = ifelse(age_mat %in% 5:9, 1,
                                                         ifelse(age_mat %in% 10:15, 2, 3))) %>%
                            count(age_category, litter_size, sex) %>%
                            mutate(prop = ifelse(litter_size == 1, n/nrow(data_cub %>% filter(litter_size == 1)),
                                                 ifelse(litter_size == 2, n/nrow(data_cub %>% filter(litter_size == 2)),
                                                        ifelse(litter_size == 3, n/nrow(data_cub %>% filter(litter_size == 3)), NA)))) %>%
                            arrange(litter_size, sex) %>%
                            pull(prop))
...
res <- rbind(fit_PA_CR$samples$chain1[, ],
             fit_PA_CR$samples$chain2[, ])
load("data/log_mass.RData")
log_mass_cub_vector <- log_mass_cub_vector[1:410]

singleton <- c(1, 0, 0) ; twin <- c(0, 1, 0) ; triplet <- c(0, 0, 1)
cub_mass <- matrix(NA, nrow = dim(res)[1], ncol = 3,
                   dimnames = list(1:dim(res)[1], c("singleton", "twin", "triplet")))
for (i in 1:3) {                 # Litter size
  for (j in 1:dim(res)[1]) {   # MCMC iteration
    cub_mass[j, i] <- singleton[i] * (res[j, "delta[1]"] * prop_age_class_sex[1] +
                                        res[j, "delta[2]"] * prop_age_class_sex[2] +
                                        res[j, "delta[3]"] * prop_age_class_sex[3] +
                                        (res[j, "delta[1]"] + res[j, "delta[7]"]) * prop_age_class_sex[4] +
                                        (res[j, "delta[2]"] + res[j, "delta[7]"]) * prop_age_class_sex[5] +
                                        (res[j, "delta[3]"] + res[j, "delta[7]"]) * prop_age_class_sex[6]) + 
      
      twin[i] * (res[j, "delta[10]"] * prop_age_class_sex[7]+
                   res[j, "delta[11]"] * prop_age_class_sex[8] +
                   res[j, "delta[12]"] * prop_age_class_sex[9] +
                   (res[j, "delta[10]"] + res[j, "delta[19]"]) * prop_age_class_sex[10] +
                   (res[j, "delta[11]"] + res[j, "delta[19]"]) * prop_age_class_sex[11] +
                   (res[j, "delta[12]"] + res[j, "delta[19]"]) * prop_age_class_sex[12]) +
      
      triplet[i] * (res[j, "delta[13]"] * prop_age_class_sex[13]+
                      res[j, "delta[14]"] * prop_age_class_sex[14] +
                      res[j, "delta[15]"] * prop_age_class_sex[15] +
                      (res[j, "delta[13]"] + res[j, "delta[19]"]) * 0 +  # no young female with a triplet male cub
                      (res[j, "delta[14]"] + res[j, "delta[19]"]) * prop_age_class_sex[16] +
                      (res[j, "delta[15]"] + res[j, "delta[19]"]) * prop_age_class_sex[17]) 
...
relative_diff <-  cub_mass_bt[, 2]/ cub_mass_bt[, 3]
mean(relative_diff)
quantile(relative_diff, probs = c(0.025, 0.975))
sum(relative_diff > 1) / length(relative_diff)
```
...
litter_mass <- cub_mass_bt
litter_mass[, 2] <- 2 * cub_mass_bt[, 2]
litter_mass[, 3] <- 3 * cub_mass_bt[, 3]

apply(litter_mass, 2, mean)

# Diff between singleton and twin
relative_diff <- (litter_mass[, 2]/ litter_mass[, 1])
mean(relative_diff)
quantile(relative_diff, probs = c(0.025, 0.975))
sum(relative_diff > 1) / length(relative_diff)

# Diff between twin and triplet
relative_diff <- (litter_mass[, 3]/litter_mass[, 2])
mean(relative_diff)
quantile(relative_diff, probs = c(0.025, 0.975))
sum(relative_diff > 1) / length(relative_diff)
```
...
load("outputs/fit_PA_CR.RData")
N <- dim(fit_PA_CR$samples$chain1)[1]
res <- rbind(fit_PA_CR$samples$chain1[seq(1, N, by = 3), ],
             fit_PA_CR$samples$chain2[seq(1, N, by = 3), ])
```
...
log_twins <- array(data = NA, dim = c(dim(res)[1], lengthgrid, 3),
                   dimnames = list(1:dim(res)[1], grid, c("young", "middle-aged", "old")))
log_triplets <- array(data = NA, dim = c(dim(res)[1], lengthgrid, 3),
                      dimnames = list(1:dim(res)[1], grid, c("young", "middle-aged", "old")))
mAgeY <- c(1, 0, 0) ; mAgeM <- c(0, 1, 0) ; mAgeO <- c(0, 0, 1)
...
cub_mass_girth <- array(NA, dim = c(dim(res)[1], lengthgrid, 3),
                        dimnames = list(1:dim(res)[1], grid, c("singleton", "twin", "triplet")))
singleton <- c(1, 0, 0) ; twin <- c(0, 1, 0) ; triplet <- c(0, 0, 1) 
for (i in 1:3) {                 # litter size 
...
cub_mass_size <- array(NA, dim = c(dim(res)[1], lengthgrid, 3),
                       dimnames = list(1:dim(res)[1], grid, c("singleton", "twin", "triplet")))
singleton <- c(1, 0, 0) ; twin <- c(0, 1, 0) ; triplet <- c(0, 0, 1) 
for (i in 1:3) {                 # litter size 
  for (j in 1:lengthgrid)   {    # Covariate value
...
cub_survival <- array(NA, dim = c(dim(res)[1], lengthgrid, 3),
                      dimnames = list(1:dim(res)[1], grid, c("young", "middle-aged", "old")))
mAgeY <- c(1, 0, 0) ; mAgeM <- c(0, 1, 0) ; mAgeO <- c(0, 0, 1)
for(i in 1:3) {                # Maternal age
  for (j in 1:lengthgrid) {    # Covariate value
...
mAgeY <- c(1, 0, 0) ; mAgeM <- c(0, 1, 0) ; mAgeO <- c(0, 0, 1)
twin <- c(0, 1)
cub_survival <- array(NA, dim = c(dim(res)[1], 2, 3),
                      dimnames = list(1:dim(res)[1], c("singleton", "twin"), c("young", "middle-aged", "old")))
for (i in 1:3) {                 # Maternal age
