...
check_convergence <- function(params.plot, nimble_output) {
  ...
  chain1 <- data.frame(nimble_output$chain1) %>%
    dplyr::select(any_of(params.plot)) %>%
    mutate(chain = "1",
           iteration = seq(1, dim(nimble_output$chain1)[1], by = 1))
  chain2 <- data.frame(nimble_output$chain2) %>%
    dplyr::select(any_of(params.plot)) %>%
    mutate(chain = "2",
           iteration = seq(1, dim(nimble_output$chain2)[1], by = 1))
...
# Get the rows and columns in the capture history matrix that correspond to a 
# a female captured with a litter-of-the-year
...
mother <- rep(423, times = nrow(CH)) # I use 423 because there is no NA on that line
# in age (matrix). This way, there won't be NAs in the model
...
# Create vectors with sex and space-use strategy
...
# Create a matrix with the day of capture
...
# We want ones where we know an individual is alive (between its first and last capture)
...
Running the model takes >15 hours. The output of the model (fit_PA_CR) is provided in the repository as a .RData file.
...
#                            monitors = params)
# 
# MCMC_conf$removeSamplers(c('theta[1]', 'theta[2]', 'theta[3]', 'theta[4]', 'theta[5]', 'theta[7]', 'theta[8]',
#                            'theta[12]', 'theta[13]', 
#                            'theta[14]', 'theta[15]', 'theta[16]', 'theta[17]','theta[18]', 'lambda[2]', 'lambda[3]', 'lambda[4]'))
# 
# # Add block sampler (Automated Factor Slice Sampler https://r-nimble.org/better-block-sampling-in-mcmc-with-the-automated-factor-slice-sampler#bib-turek17)
# MCMC_conf$addSampler(target = c('theta[1]', 'theta[2]', 'theta[3]', 'theta[4]', 'theta[5]', 'theta[7]', 'theta[8]'), type = 'AF_slice')
# MCMC_conf$addSampler(target = c('theta[12]', 'theta[13]'), type = 'AF_slice')
# MCMC_conf$addSampler(target = c('theta[14]', 'theta[15]', 'theta[16]', 'theta[17]','theta[18]', 
#                                 'lambda[2]', 'lambda[3]', 'lambda[4]'), type = 'AF_slice')
# 
# end <- Sys.time() ; end ; end - start
# 
# # Compile MCMC
